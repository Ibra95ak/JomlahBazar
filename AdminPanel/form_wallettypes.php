<?php
//Get Wallet type class
require_once 'libraries/Ser_Wallettypes.php';
$db = new Ser_Wallettypes();
$err=-1;

if(isset($_GET['wallettypeId'])) $wallettypeId=$_GET['wallettypeId'];
else $wallettypeId=0;

if($wallettypeId>0){
    //Edit wallet
    $get_wallettype=$db->GetWalletTypeById($wallettypeId);
    if($get_wallettype){
        $wallettypeId=$get_wallettype['wallettypeId'];
        $wallettype=$get_wallettype['wallettype'];
        $active=$get_wallettype['active'];
    }else{
        $wallettypeId='';
        $wallettype='';
        $active='';
            ...
            <div class="form-group row">
                <div class="col-lg-4">
                    ...
                    <div class="input-group">
                        <div class="input-group-prepend"><span class="input-group-text"><i
                                    class="la la-user"></i></span></div>
                        <input type="text"  class="form-control" placeholder="" name="wallettype" id="wallettype" value="<?php if(isset($wallettype)) echo $wallettype;else echo '';?>">
                    </div>
                </div>
            </div> 
...
$('#btn_submit').click(function(e) {
    ...
    $.ajax({
        type: "POST",
        url: "http://localhost/JomlahBazar/AdminPanel/controllers/cu/cu_wallettype.php",
        cache: false,
        contentType: false,
        processData: false,
        data: formdata1,
                ...
                case 0:
                    ...
                    setTimeout(function() {
                        ...
                        window.location.replace(
                            "http://localhost/JomlahBazar/AdminPanel/por_wallettype.php"
                        );
                    }, 2000);
                    break;
                case 1:
