	...
	var demo = function() {
		...
		var datatable = $('.kt-datatable').KTDatatable({
			...
			data: {
				...
				source: {
					read: {
						url: DIR_CONT+DIR_ORD+'CON_Orders.php?action=seller_orders',
					},
				},
            ...
            template: function(row) {
                ...
                return '<span class="kt-badge kt-badge--' + status[row.active].state + ' kt-badge--dot"></span>&nbsp;<span class="kt-font-bold kt-font-' + status[row.active].state + '">' + status[row.active].title + '</span>';
					...
					template: function(row) {
						return '\
							<a href=DIR_VIEW+DIR_ORD+"seller_own_orders_detail.php?orderId='+row.orderId+'" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit details">\
