...
if ($action=='get-mcat-cat-dropdown') {
  $dropdown='<option value="0">Choose Category</option>';
  if($maincategoryId!=0){
      $categories = $dbmc->GetCategoriesByMaincategoryId($maincategoryId);
      if ($categories) {
        foreach ($categories as $category) {
          if ($category['categoryId']==$categoryId) {
            $dropdown.='<option value="'.$category['categoryId'].'" data-pt="'.$category['maincategory'].'" selected>'.$category['name'].'</option>';
          }else {
            $dropdown.='<option value="'.$category['categoryId'].'" data-pt="'.$category['maincategory'].'">'.$category['name'].'</option>';
          }
        }
      }
  }
