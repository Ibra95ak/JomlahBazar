...
if ($action=='post') {
    ...
    if($review){
      $response['err']=0;/*successfull add*/
      /*update product review*/
      $product_reviews = $db->GetReviewsByproductId($productId);
      foreach ($product_reviews as $product_review) {
        $count++;
        $ranking+=$product_review['stars'];
      }
      $product_ranking=$ranking/$count;
      $product_rank = $db->UpdateProductRanking($productId,$product_ranking);
    }
...
if ($action=='delete') {
  ...
  header("location:".DIR_ROOT."?err=".$response['err']);
}
